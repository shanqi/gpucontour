...
int findPitchInFloats(int width) {
  ...
  cudaMallocPitch((void**)&test, &pitch, width * sizeof(float), 1);
  cudaFree(test);
...
/* /\*   printf("\n"); *\/ */
  ...
  cudaMalloc((void**)&devSum, width * sizeof(float) * height);
  float* devRSqrtSum;
  //CUDA_SAFE_CALL(cudaMalloc((void**)&devRSqrtSum, width * sizeof(float) * height));
  cudaMalloc((void**)&devRSqrtSum, width * sizeof(float) * height);
